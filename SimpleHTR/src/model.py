...
class Model:
    ...
    def __init__(self,
                 ...
                 corpus_path = './data/corpus.txt') -> None:
    ...
    def setup_ctc(self) -> None:
        ...
        # word beam search decoding (see https://github.com/githubharald/CTCWordBeamSearch)
            ...
            word_char_path = self.model_dir + '/wordCharList.txt'
            with open(word_char_path, encoding='utf-8') as f:
                word_chars = f.read().splitlines()[0]
    ...
    def setup_tf(self, model_dir='./model/') -> Tuple[tf.compat.v1.Session, tf.compat.v1.train.Saver]:
        ...
        # Use separate savers for restore and save
        if self.fine_tune:
            # Restore: skip Variable_5
            to_restore = [v for v in tf.compat.v1.global_variables() if 'Variable_5' not in v.name]
            restore_saver = tf.compat.v1.train.Saver(var_list=to_restore)
            # Save: include everything
            save_saver = tf.compat.v1.train.Saver(max_to_keep=1)
        else:
            restore_saver = save_saver = tf.compat.v1.train.Saver(max_to_keep=1)

        if latest_snapshot:
            print('Init with stored values from ' + latest_snapshot)
            restore_saver.restore(sess, latest_snapshot)

            # fine-tuning: reinitialize the new projection layer
            if self.fine_tune:
                proj_vars = [v for v in tf.compat.v1.global_variables() if 'Variable_5' in v.name]
                print('[DEBUG] Reinitializing:', [v.name for v in proj_vars])
                sess.run(tf.compat.v1.variables_initializer(proj_vars))
        else:
            print('Init with new values')
            sess.run(tf.compat.v1.global_variables_initializer())

        # Store the saver for saving later
        self.saver = save_saver
        return sess, save_saver
